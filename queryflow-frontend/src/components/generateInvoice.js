...
export const generateInvoice = ({ sale, seller, buyer, invoiceNumber }) => {
  ...
  autoTable(doc, {
    startY: 105,
    head: [["#", "Item Description", "HSN/SAC", "Qty", "Unit Price (Rs.)", "Amount (Rs.)"]],
    body: [
      [
        "1",
        ...
        `Rs.${sale.sellPrice?.toLocaleString("en-IN")}`,
  ...
  autoTable(doc, {
    startY: finalY,
    body: [
      ["", "", "", "", "Subtotal:", `Rs.${baseAmount.toLocaleString("en-IN")}`],
      ["", "", "", "", "CGST (9%):", `Rs.${(gstAmount / 2).toLocaleString("en-IN")}`],
      ["", "", "", "", "SGST (9%):", `Rs.${(gstAmount / 2).toLocaleString("en-IN")}`],
  ...
  doc.text(
    `Cost Price: Rs.${sale.costPrice?.toLocaleString("en-IN")} | Net Profit on this sale: Rs.${sale.profit?.toLocaleString("en-IN")}`,
    14,
    profitY
  );
