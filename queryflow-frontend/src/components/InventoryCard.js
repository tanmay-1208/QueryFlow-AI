    ...
    <ScaleIn>
      <div className={`bg-[#131313] p-6 rounded-[2.5rem] border transition-all shadow-xl relative group ${isLowStock ? 'border-red-500/20' : 'border-white/5 hover:bg-[#181818]'}`}>
        ...
        <button
          ...
          className="absolute top-4 right-4 md:top-6 md:right-6 w-[44px] h-[44px] flex items-center justify-center text-gray-400 md:text-gray-700 hover:text-red-500 material-symbols-outlined text-sm opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10"
        >
          delete
        </button>
        ...
        <button
          ...
          className="absolute top-4 right-[50px] md:top-6 md:right-12 w-[44px] h-[44px] flex items-center justify-center text-gray-400 md:text-gray-700 hover:text-[#4182ff] material-symbols-outlined text-sm opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10"
        >
          edit
        </button>
        ...
        <div className="flex justify-between items-start mb-6 pr-16">
          <div className="w-full">
            ...
            {item.category && (
              <span className="inline-block mt-2 bg-[#4182ff]/10 text-[#4182ff] text-[14px] md:text-[7px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border border-[#4182ff]/20">
                {item.category}
              </span>
            )}
          </div>
          {isLowStock && (
            <span className="absolute top-2 left-6 bg-red-500/10 text-red-500 text-[14px] md:text-[7px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest animate-pulse">
              Low Stock
            </span>
          )}
        </div>
        ...
        <div className="mb-6 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-black/40 p-2 rounded-xl border border-white/5 flex flex-col items-start px-3">
              <span className="text-[14px] md:text-[6px] text-[#4182ff] uppercase font-black mb-1 opacity-80">Retail</span>
        ...
        <div className="flex justify-between items-center bg-black/40 px-5 py-3 min-h-[44px] rounded-2xl mb-6 border border-white/5">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 bg-white/5 text-white py-3 min-h-[44px] rounded-xl font-black uppercase text-[14px] md:text-[9px] tracking-widest hover:bg-[#00ff88]/10 hover:text-[#00ff88] transition-all"
          >
            Sell
          </button>
        </div>
