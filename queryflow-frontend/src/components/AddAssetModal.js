...
const AddAssetModal = ({ isOpen, onClose, onAdd, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // THE FALLBACK: If server is down, add to local list anyway
      if (window.confirm("Backend unreachable. Sync to local session?")) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-6">
      <div className="bg-[#1c1b1b] w-full max-w-md p-10 rounded-[3rem] border border-white/10 shadow-2xl">
        <h3 className="text-2xl font-black uppercase tracking-tighter mb-8 italic text-white text-center">Vault Asset</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input className="w-full bg-[#2a2a2a] p-4 rounded-2xl border-none outline-none text-white text-[11px] font-black uppercase" placeholder="Units" type="number" onChange={e => setForm({...form, stock: e.target.value})} required />
          <div className="flex gap-4 pt-6">
            ...
            <button type="submit" className="flex-1 bg-[#4182ff] py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest text-white">Execute</button>
          </div>
        </form>
      </div>
    </div>
