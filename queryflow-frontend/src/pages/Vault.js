...
const Vault = ({ userId, onLogout }) => {
  ...
  const [ledger, setLedger] = useState([
    { id: 1, type: 'SYS', action: 'Terminal Sync', entity: 'NODE-04', val: 'Success', time: '08:29' }
  ]);
            ...
            <div className="animate-in fade-in space-y-8">
              ...
              <div className="grid grid-cols-4 gap-4">
                {[
                  ...
                  { label: "Projected Profit", val: `$${Math.floor(net).toLocaleString()}`, color: "text-[#66dd8b]" },
                  { label: "Tax (18%)", val: `-$${Math.floor(tax).toLocaleString()}`, color: "text-red-900/40" },
                  ...
                  <div key={i} className="bg-[#131313] p-5 rounded-[2rem] border border-white/5 shadow-2xl">
                    ...
                    <h3 className={`text-lg font-black truncate tracking-tighter ${s.color}`}>{s.val}</h3>
                  </div>
                ))}
              </div>
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-[#1c1b1b] p-10 rounded-[3rem] border border-white/5 relative overflow-hidden flex flex-col justify-between h-72 shadow-2xl">
