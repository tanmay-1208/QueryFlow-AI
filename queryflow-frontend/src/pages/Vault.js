...
const Vault = ({ userId, onLogout }) => {
  ...
  const [ledger, setLedger] = useState([
    { id: 1, type: 'SYS', action: 'Terminal Sync', entity: 'NODE-04', val: 'Success', time: '08:21' }
  ]);
  ...
  const updateStock = async (id, delta) => {
    ...
    if (!item) return;
    const newStock = Math.max(0, safeVal(item.stock) + delta);
    setItems(prev => prev.map(i => i.id === id ? { ...i, stock: newStock } : i));
    setLedger(prev => [{ id: Date.now(), type: delta > 0 ? 'IN' : 'OUT', action: delta > 0 ? 'Restock' : 'Sale', entity: item.name, val: `${delta > 0 ? '+' : ''}${delta}`, time: 'Now' }, ...prev.slice(0, 4)]);
    try { await axios.put(`${API_BASE_URL}/api/products/${id}`, { stock: newStock, userId }); } catch (err) { console.error(err); }
  };

  const deleteAsset = async (id) => {
    if (!window.confirm("Permanent Liquidation: Are you sure?")) return;
    setItems(prev => prev.filter(i => i.id !== id));
    try { await axios.delete(`${API_BASE_URL}/api/products/${id}?userId=${userId}`); } catch (err) { console.error(err); }
  };

  // REFRESH HANDLER FOR THE MODAL
  const handleOnAdd = () => {
    fetchItems(); // Reload data from server
    setIsAddModalOpen(false); // Close modal
    setLedger(prev => [{ id: Date.now(), type: 'SYS', action: 'Asset Added', entity: 'TERMINAL', val: 'NEW', time: 'Now' }, ...prev.slice(0, 4)]);
  };
  ...
  if (isLoading) return <div className="h-screen bg-[#0e0e0e] flex items-center justify-center text-[#adc7ff] font-black animate-pulse uppercase tracking-[0.3em]">Decrypting Vault...</div>;
  ...
  return (
    <div className="flex h-screen w-screen bg-[#0e0e0e] text-white fixed inset-0 overflow-hidden font-['Inter']">
      ...
      <aside className="w-64 border-r border-white/5 bg-[#0e0e0e] flex flex-col p-8 shrink-0">
        ...
        <button onClick={onLogout} className="mt-auto text-red-900/40 text-[9px] font-black uppercase tracking-[0.3em] hover:text-red-500">Terminate</button>
      </aside>
          ...
          <div className="flex items-center gap-6">
            ...
            {activeTab === "inventory" && (
              <button 
                ...
                className="bg-[#4182ff] w-8 h-8 rounded-full flex items-center justify-center material-symbols-outlined text-sm hover:scale-110 transition-transform cursor-pointer"
              >
                add
              </button>
            )}
          </div>
          <input className="bg-[#131313] border border-white/5 rounded-xl px-4 py-2 text-[10px] w-64 outline-none uppercase font-black text-gray-400" placeholder="Search Terminal..." onChange={e => setSearchTerm(e.target.value)} />
        </header>
            ...
            <div className="animate-in fade-in space-y-10">
              <div className="grid grid-cols-4 gap-4">
                {[
                  ...
                  { label: "Tax Provision", val: `-$${Math.floor(taxReserve).toLocaleString()}`, col: "text-red-900/40" },
                  { label: "Active Assets", val: items.length, col: "text-white" }
                ].map((stat, i) => (
                  <div key={i} className="bg-[#131313] p-6 rounded-3xl border border-white/5 shadow-2xl overflow-hidden">
                    <span className="text-[9px] text-gray-600 uppercase font-black tracking-widest block mb-2">{stat.label}</span>
                    <h3 className={`text-xl font-black truncate tracking-tight ${stat.col}`}>{stat.val}</h3>
                  </div>
                ))}
              </div>
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-[#1c1b1b] p-10 rounded-[3rem] border border-white/5 relative overflow-hidden flex flex-col justify-between h-80 shadow-2xl">
                  ...
                  <h3 className="text-7xl font-black tracking-tighter leading-none text-white truncate max-w-full">${(totalValuation - taxReserve).toLocaleString()}</h3>
              ...
              <div className="bg-[#131313] rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl">
                ...
                <div className="p-2 space-y-1">
                  {ledger.map(entry => (
                    <div key={entry.id} className="grid grid-cols-4 items-center p-4 hover:bg-white/[0.01] rounded-xl transition-all">
                      <span className="text-[10px] font-black text-gray-700">{entry.time}</span>
                      <span className="text-[9px] w-fit bg-white/5 px-2 py-0.5 rounded text-gray-600 uppercase font-black">{entry.type}</span>
                      <span className="text-[10px] font-bold uppercase truncate">{entry.action} <span className="text-[#4182ff] ml-1">[{entry.entity}]</span></span>
            ...
            <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom-4">
               <div className="bg-[#131313] p-16 rounded-[4rem] border border-white/5 shadow-2xl text-center">
                  ...
                  <div className="space-y-6 border-t border-white/5 pt-12 text-left">
                     ...
                     <div className="flex justify-between items-center text-gray-400 text-xs font-bold uppercase tracking-widest">
                       <span>Calculated Tax Liability (18%)</span>
                       <span className="text-red-900/40 text-2xl tracking-tighter">-${Math.floor(taxReserve).toLocaleString()}</span>
                     </div>
      ...
      {/* CRITICAL FIX: Ensure all props are passed correctly to AddAssetModal */}
      <AddAssetModal 
