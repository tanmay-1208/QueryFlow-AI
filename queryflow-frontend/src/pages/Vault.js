...
const Vault = ({ userId, onLogout }) => {
  ...
  const [ledger, setLedger] = useState([
    { id: 1, type: 'SYS', action: 'Terminal Sync', entity: 'NODE-04', val: 'Success', time: '08:12' }
  ]);
  ...
  const handleChat = (e) => {
    ...
    setUserQuery("");
    setTimeout(() => {
      setChatHistory([...newHist, { role: 'assistant', text: `Audit Complete. Asset Valuation: $${totalValuation.toLocaleString()}.` }]);
    }, 800);
  };

  if (isLoading) return <div className="h-screen bg-[#0e0e0e] flex items-center justify-center text-[#adc7ff] font-black animate-pulse">BOOTING TERMINAL...</div>;
  ...
  return (
    <div className="flex h-screen w-screen bg-[#0e0e0e] text-white fixed inset-0 overflow-hidden font-['Inter']">
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[#0a0a0a]">
        <header className="min-h-20 border-b border-white/5 flex justify-between items-center px-10">
          <div className="flex items-center gap-6">
            ...
            {/* RESTORED: ADD BUTTON */}
            {activeTab === "inventory" && (
              <button onClick={() => setIsAddModalOpen(true)} className="bg-[#4182ff] w-8 h-8 rounded-full flex items-center justify-center material-symbols-outlined text-sm hover:scale-110 transition-transform">add</button>
            )}
          </div>
            ...
            <div className="animate-in fade-in space-y-10">
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-[#1c1b1b] p-10 rounded-[3rem] border border-white/5 relative overflow-hidden flex flex-col justify-between h-80 shadow-2xl">
                  ...
                  <h3 className="text-7xl font-black tracking-tighter leading-none text-white">${(totalValuation - taxReserve).toLocaleString()}</h3>
                  <div className="absolute bottom-0 left-0 h-1.5 w-[80%] bg-[#4182ff] shadow-[0_0_20px_#4182ff]"></div>
                </div>

                {/* RESTORED: TOP 5 INDEX */}
      ...
      <aside className="w-80 border-l border-white/5 bg-[#0e0e0e] flex flex-col p-8 shrink-0 shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar mb-4">
          {chatHistory.map((msg, i) => (
            <div key={i} className={`p-5 rounded-2xl text-[10px] leading-relaxed ${msg.role === 'assistant' ? 'bg-[#131313] border border-white/5 text-gray-400' : 'bg-[#4182ff]/10 text-[#4182ff]'}`}>{msg.text}</div>
          ))}
          <div ref={chatEndRef} />
        </div>
      ...
      <AddAssetModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onAdd={fetchItems} userId={userId} />
    </div>
  );
};
