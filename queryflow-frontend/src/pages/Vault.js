...
const Vault = ({ userId, onLogout }) => {
  ...
  const handleChat = (e) => {
    ...
    setUserQuery("");
    setTimeout(() => {
      setChatHistory([...newHist, { role: 'assistant', text: `Audit Complete. Current Liquidity: $${totalValuation.toLocaleString()}. System status Bullish.` }]);
    }, 800);
  };
    ...
    <div className="flex h-screen w-screen bg-[#0e0e0e] text-white fixed inset-0 overflow-hidden font-['Inter']">
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[#0a0a0a]">
        <header className="min-h-20 border-b border-white/5 flex justify-between items-center px-10">
          ...
          <input className="bg-[#131313] border border-white/5 rounded-xl px-4 py-2 text-[10px] w-64 outline-none uppercase font-black tracking-widest text-gray-400 focus:border-white/10" placeholder="Global Search..." onChange={e => setSearchTerm(e.target.value)} />
        </header>
            ...
            <div className="animate-in fade-in space-y-10">
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-[#1c1b1b] p-10 rounded-[3rem] border border-white/5 relative overflow-hidden flex flex-col justify-between h-80 shadow-2xl">
                  ...
                  <h3 className="text-6xl font-black tracking-tighter leading-none text-white">${(totalValuation - taxReserve).toLocaleString()}</h3>
          ...
          {activeTab === "reports" && (
            <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom-4">
               <div className="bg-[#131313] p-16 rounded-[4rem] border border-white/5 shadow-2xl">
                  <h3 className="text-4xl font-black mb-12 uppercase tracking-tighter text-center">Fiscal Audit</h3>
      ...
      <aside className="w-80 border-l border-white/5 bg-[#0e0e0e] flex flex-col p-8 shrink-0 shadow-2xl">
        ...
        <form onSubmit={handleChat} className="relative mt-auto">
          <input 
            className="w-full bg-[#131313] border border-white/5 rounded-xl px-4 py-3 text-[10px] text-white outline-none focus:border-[#4182ff]/30 transition-all" 
