...
const LandingPage = () => (
  <div className="landing-wrapper">
    <div className="hero-content">
      ...
      <p className="hero-subtitle">Professional stock tracking and AI-driven insights.</p>
        ...
        {filteredItems.map((item) => {
          ...
          return (
            <div key={item.id} className={`inventory-card ${topPerformer?.id === item.id ? 'highlight-margin' : ''} ${isLowStock ? 'low-stock-warning' : ''}`}>
              ...
              <h4 className="item-title">{item.name || "Unnamed Item"}</h4>
              ...
              <div className={`item-meta ${isLowStock ? 'text-red bold' : ''}`}>
                Stock: {item?.stock || 0} {isLowStock ? '⚠️' : ''} | Sold: {item?.sold_count || 0}
              </div>
              
              <div className="item-actions">
                <button onClick={() => handleAction('sell', item.id)} className="log-sale-btn">Sell</button>
                <button onClick={() => handleAction('restock', item.id)} className="restock-btn">Stock</button>
...
const Advisor = () => {
  ...
  return (
    <div className="dashboard-container">
      <h2 className="page-header">Financial Advisor</h2>
