...
const LandingPage = () => (
  <div className="bg-[#131313] min-h-screen text-white font-['Inter'] flex items-center justify-center text-center p-8">
    <div className="max-w-4xl">
      <h1 className="text-7xl md:text-8xl font-black font-['Manrope'] tracking-tighter leading-tight mb-8 italic">QueryFlow <span className="text-[#adc7ff] not-italic">Vault</span></h1>
...
// --- THE VAULT TERMINAL (LIVE SYNC EDITION) ---
const Vault = ({ userId, onLogout }) => {
  ...
  // --- UPDATED STOCK UPDATE WITH LEDGER SYNC ---
  const updateStock = async (id, delta) => {
    ...
    const newEntry = {
      id: Date.now(),
      action: delta > 0 ? "Inventory Restock" : "Asset Liquidation",
      entity: item.name,
      status: "Verified",
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-[#1c1b1b] p-7 rounded-3xl border-l-4 border-[#66dd8b] shadow-xl overflow-hidden">
                  ...
                  <h3 className="text-2xl font-black font-['Manrope'] text-[#66dd8b] tracking-tighter truncate" title={`$${netProfit.toLocaleString()}`}>${netProfit.toLocaleString()}</h3>
                </div>
                <div className="bg-[#1c1b1b] p-7 rounded-3xl border-l-4 border-gray-700 shadow-xl overflow-hidden">
                  <span className="text-[10px] text-gray-500 uppercase font-['Space_Grotesk'] tracking-widest block mb-1">Alerts</span>
              ...
              <div className="bg-[#1c1b1b] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
                ...
                <div className="space-y-4">
                  {ledger.map((entry) => (
                    <div key={entry.id} className="flex justify-between items-center py-4 border-b border-white/5 last:border-0 animate-in slide-in-from-top duration-500">
                      <div>
